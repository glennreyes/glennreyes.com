...
Since these are represented in decibels, we specify a range of what we want to capture from, the minimum and maximum volume, because human ears can only hear (that's what we care about mostly) from 20 to 20,000 Hertz (Hz).

Finally, we capture the most dominant sample and convert it to a frequency (or pitch). It involves a bit of math, but here's how it looks:
