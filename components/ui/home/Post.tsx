...
export function Post({ post }: PostProps) {
  ...
  return (
    <article className="group relative grid gap-3 py-6">
      <div className="relative z-10 px-4 text-gray-400">
      ...
      <h3 className="px-4 text-lg font-semibold tracking-tight">
        <Link href={`/posts/${post.slug}`}>
          <span className="md:-inset-z-2 absolute inset-0 z-20" />
          <span className="relative z-10">{post.title}</span>
        </Link>
      </h3>
      <p className="relative z-10 px-4 text-stone-500">{post.excerpt}</p>
      <div className="absolute inset-0 scale-95 bg-stone-50 opacity-0 transition group-hover:scale-100 group-hover:opacity-100 sm:rounded-2xl md:-inset-x-2" />
    </article>
  );
}
