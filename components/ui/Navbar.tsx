...
export function Navbar({ children, isScrollTop = false, ...props }: NavbarProps) {
  const classes = clsx('fixed top-0 inset-x-0 transition duration-500', {
    'bg-yellow-100/75 supports-[backdrop-filter]:backdrop-blur-md': !isScrollTop,
  });
  ...
  return (
    <header className={classes}>
      <div className="p-4 md:px-8">
        <nav className="mx-auto flex max-w-7xl justify-end gap-2 md:justify-center md:gap-4" {...props}>
          {children}
        </nav>
      </div>
